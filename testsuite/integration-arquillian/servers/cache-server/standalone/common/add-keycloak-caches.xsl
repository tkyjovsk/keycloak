...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//*[local-name()='infinispan']/*[local-name()='jgroups']
        ...
        <stack name="xsite" extends="udp">
            <relay.RELAY2 xmlns="urn:org:jgroups" site="{$remote.site}"/>
            <remote-sites default-stack="tcp-with-tcpping">
    ...
    <xsl:template match="//*[local-name()='infinispan']/*[local-name()='jgroups']">
        <xsl:copy>
            <stack-file name="tcp-with-tcpping" path="jgroups-tcp-with-tcpping.xml"/> 
            <xsl:apply-templates select="@*|node()" />
        </xsl:copy>
    </xsl:template>
