  ...
  def loginThroughLoginForm() : AdminConsoleScenarioBuilder = {
    chainBuilder = chainBuilder
      .exec(http("JS Adapter Auth - Login Form Redirect")
        .get("${keycloakServer}/realms/master/protocol/openid-connect${keycloakServer}?client_id=security-admin-console&redirect_uri=${keycloakServerUrlEncoded}%2Fadmin%2Fmaster%2Fconsole%2F&state=${state}&nonce=${nonce}&response_mode=fragment&response_type=code&scope=openid")
        .headers(UI_HEADERS)
    ...
    chainBuilder = chainBuilder
      .exec(http("Console - user-detail.html")
        ...
        .resources(
          ...
          http("Console REST - ${realm}${keycloakServer}entication/required-actions")
            .get("${keycloakServer}/admin/realms/${realm}${keycloakServer}entication/required-actions")
            .headers(AUTHORIZATION)
            .check(status.is(200)),
