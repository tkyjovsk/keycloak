...
//    println("search "+listEndpoint(s))
  ...
  def httpPost = http("${postRequestName}")
...
//    println("create "+listEndpoint(s)+" ---> "+entity(s))
  ...
  }.//doIfOrElse(s => entity(s).getId == null) (
  exec ( httpPost )
...
//  ) ( // if id already set just update the entity
//    update
//  )
...
//    println("update "+instanceEndpoint(s))
  ...
  def updateEntities:ChainBuilder = forEach(update)
