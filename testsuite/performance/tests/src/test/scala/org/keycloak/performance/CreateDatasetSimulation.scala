...
class CreateDatasetSimulation extends Simulation  with AdminCLI  {
  ...
  setUp(
    ...
    .exec( 
      ...
      ResourceServers.iterator(dataset.resourceServers().iterator),
      ResourceServers.updateEntities, 
      Sync.waitForOthers("authz resource servers"),
      
      Scopes.iterator(dataset.scopes().iterator),
      Scopes.createEntities, 
      Sync.waitForOthers("authz scopes"),
      
      Resources.iterator(dataset.resources().iterator),
      Resources.createEntities, 
      Sync.waitForOthers("authz resources"),
